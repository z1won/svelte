<script>
  ...
  const collectGraphqlVideos = `async({page})=>page.evaluate(async()=>{const s=location.pathname.split('/').filter(Boolean)[1],A='ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_';let n=0;for(const c of s)n=n*64+A.indexOf(c);const t=(document.documentElement.innerHTML.match(/\["LSD",\[\],\{"token":"([^"]+)/)||[])[1];if(!t)return[];const r=await fetch('/api/graphql',{method:'POST',headers:{'x-fb-friendly-name':'PolarisLoggedOutDesktopWWWPostRootContentQuery','x-fb-lsd':t},body:new URLSearchParams({lsd:t,fb_api_caller_class:'RelayModern',fb_api_req_friendly_name:'PolarisLoggedOutDesktopWWWPostRootContentQuery',server_timestamps:'true',variables:JSON.stringify({media_id:String(n)}),doc_id:'27130156389949648'})});const p=(await r.json())?.data?.xig_polaris_media?.if_not_gated_logged_out,a=p?.carousel_media||[p];return a.flatMap(x=>(x?.video_versions||[]).map(v=>({url:v.url,kind:'video',width:v.width,height:v.height})));})`;
  ...
  async function requestExtraction(url, useEmbed=false, graphql=false){
    const target = useEmbed ? `${url.replace(/\/?([?#].*)?$/,'')}/embed/` : url;
    const fn = graphql ? collectGraphqlVideos : collectMedia;
  ...
  async function extract(){
    ...
    loading=true;status=videoRequest?'릴스 동영상 원본을 분석하고 있어요':'공개 미디어를 분석하고 있어요';tried=videoRequest?'동영상 원본 → GraphQL 스트림 → 공개 임베드 순서로 확인':'원본 미디어 → 이미지 후보 순서로 확인';
    try{
      let found=await requestExtraction(value,false);
      if(videoRequest && !found.some(x=>x.kind==='video')){status='Instagram GraphQL에서 동영상 스트림을 확인하고 있어요';found=[...(await requestExtraction(value,false,false,true)),...found]}
      if(videoRequest && !found.some(x=>x.kind==='video')){status='공개 임베드에서 동영상 원본을 다시 확인하고 있어요';found=[...(await requestExtraction(value,true)),...found]}
      if(!found.length || (videoRequest && !found.some(x=>x.kind==='video')))throw new Error('no video');
      media=found.slice(0,20);selected=0;title=videoRequest?'Instagram 공개 동영상':'Instagram 공개 미디어';author='Instagram';
      const videos=media.filter(x=>x.kind==='video').length;status=`${media.length}개 미디어 확인 · ${videos ? `${videos}개 동영상` : '이미지'}`;tried='';
    }catch(e){error=videoRequest?'릴스 동영상 원본을 찾지 못했어요.':'Instagram 공개 미디어를 찾지 못했어요.';tried=videoRequest?'썸네일 이미지는 결과로 사용하지 않습니다. 공개 릴스이고 브라우저에서 재생 가능하면 다시 시도해 주세요.':'로그인/접근 제한이 있는 콘텐츠는 추출할 수 없습니다.'}
...
<main class="page"><div class="ambient ambient-one"></div><div class="ambient ambient-two"></div><section class="shell">
  ...
  <div class="hero"><div class="hero-kicker"><span>01</span> INSTAGRAM MEDIA EXTRACTOR</div><h1>공개 미디어를<br/><em>빠르게 저장하세요.</em></h1><p>사진, 캐러셀, 공개 릴스까지 한 링크에서 확인합니다.<br class="desktop"/> 릴스는 썸네일을 제외하고 동영상 원본만 결과로 보여줘요.</p></div>
  <section class="extract-card"><div class="card-topline"><div><span class="step-dot">1</span><span>Instagram 링크</span></div><button type="button" class="paste" on:click={pasteLink}>{copied?'붙여넣음 ✓':'클립보드 붙여넣기'}</button></div>
    ...
    <div class="helper"><span>✦</span> 릴스는 동영상 원본 우선 · 공개 게시물 · 캐러셀 지원 · 로그인/비공개 콘텐츠 제외</div>
  </section>
